...
function loadAssets(g, assets)
{
  ...
  {
    if ("image" == assets[i].type)
    {
      eval("g." + assets[i].slug + " = new Image();"), eval("g." + assets[i].slug + '.src = "' + assets[i].src + '";'), eval("g." + assets[i].slug + ".onload = g.loaded_items++;");
    }
...
}, Mouse.prototype.isOverPiece = function (a)
{
  ...
  {
    (b[d].y <= pt.y && pt.y < b[f].y || b[f].y <= pt.y && pt.y < b[d].y) && pt.x < (b[f].x - b[d].x) * (pt.y - b[d].y) / (b[f].y - b[d].y) + b[d].x && (c = !c);
  }
  return c
}, Mouse.prototype.isOverRect = function (a, b, c, d)
{
  ...
  for (var f = !1, g = -1, h = e.length, i = h - 1; ++g < h; i = g)
  {
    (e[g].y <= pt.y && pt.y < e[i].y || e[i].y <= pt.y && pt.y < e[g].y) && pt.x < (e[i].x - e[g].x) * (pt.y - e[g].y) / (e[i].y - e[g].y) + e[g].x && (f = !f);
  }
  return f
...
{
  var a = !1, b = this.x - this.target.x, c = this.y - this.target.y, d = b * b + c * c;
...
}, Game.prototype.placePieces = function ()
{
  ...
  {
    x = Math.floor(Math.random() * this.scaled_width * 2), y = Math.floor(Math.random() * this.scaled_height * 2), temp = new Piece(i + 1, this, this.piece_width, this.piece_height, x, y, new Point2D(this.x, this.y), new Point2D(this.holders[i].x, this.holders[i].y), this.holders[i], !0, !1), this.pieces.push(temp);
  }
  document.getElementById("audio-chimes").play()
}, Game.prototype.placeHolders = function ()
{
  var a = 1, b = Math.round(this.scaled_width - this.img_width / 2),
    c = Math.round(this.scaled_height - this.img_height / 2);
  c += 26;
  for (var d = 0; d < this.num_lines; ++d)
  {
    for (var e = 0; e < this.num_lines; ++e)
    {
      temp = new Holder(a, this, e * this.piece_width + b + this.piece_width / 2, d * this.piece_height + c + this.piece_height / 2, d, e, !1), this.holders.push(temp), a++
    }
  }
}, Game.prototype.render = function ()
{
  ...
  {
    for (var a = 0; a < this.holders.length; a++)
    {
      holder = this.holders[a], holder.draw();
    }
    for (var b = new Array, c = !1, a = 0; a < this.pieces.length; a++)
    {
      piece = this.pieces[a], !c && piece.mouse_is_over() && (c = !0), piece.placed ? piece != this.selected && piece.draw() : b.push(piece), this.selected || (!this.over || this.over.id < piece.id || piece.mouse_is_over()) && piece.mouse_is_over() && !piece.placed && (this.over = piece);
    }
...
}, Game.prototype.draw_bg = function ()
{
  ...
  var a = Math.round(this.scaled_width - this.img_width / 2), b = Math.round(this.scaled_height - this.img_height / 2);
...
{
  for (var r = this.random_image; r == this.random_image;)
  {
    this.random_image = Math.floor(12 * Math.random()) + 1, this.random_image < 10 && (this.random_image = new String("0" + this.random_image));
  }
...
}, function ()
{
  ...
  {
    var c = (new Date).getTime(), d = Math.max(0, 22 - (c - a)), e = window.setTimeout(function ()
    {
      b(c + d)
    }, d);
    return a = c + d, e
...
var game = new Game, interval = null, gameInterval = null;
game.debug = !1, m = {game: game}, interv = function ()
{
